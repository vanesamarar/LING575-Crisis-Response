...
def evaluate_backtranslations(srcs, refs, bts):
    ...
    # BLEU: sentence_bleu per pair, then average
    bleu_scores = []
    for bt, ref in zip(bts, refs):
        score = sentence_bleu(bt, [ref]).score
    ...
    return comet_avg, bleu_avg, bert_avg
...
def main():
    ...
    for provider in providers:
        provider_output_path = os.path.join(shared_eval_dir, f"{provider}_back_evaluation_results_2.txt")
